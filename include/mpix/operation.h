 ...
 * @defgroup mpix_operation mpix/operation.h
 ...
 *
 * @c mpix_operation structs are chained together into a linked list.
...
struct mpix_operation {
	...
	void (*run)(struct mpix_operation *op);
 ...
 *
 * The lines will be considered as converted as soon as @ref mpix_operation_done() is called,
 ...
 */
static inline uint8_t *mpix_operation_get_output_line(struct mpix_operation *op)
{
	...
	pitch = next->width * mpix_bits_per_pixel(next->format_in) / BITS_PER_BYTE;
 ...
 */
static inline const uint8_t *mpix_operation_get_input_lines(struct mpix_operation *op, size_t nb)
{
	uint32_t pitch;
	uint32_t size;
	uint8_t *lines;
	...
	pitch = op->width * mpix_bits_per_pixel(op->format_in) / BITS_PER_BYTE;
...
/**
 * @brief Shorthand for @ref mpix_operation_get_input_lines() to get a single input line.
 *
 * @param op Current operation in progress.
 * @return Pointer to the requested number of lines, never NULL.
 */
static inline const uint8_t *mpix_operation_get_input_line(struct mpix_operation *op)
{
	return mpix_operation_get_input_lines(op, 1);
}
 ...
 */
static inline uint8_t *mpix_operation_peek_input_line(struct mpix_operation *op)
{
	uint32_t pitch = op->width * mpix_bits_per_pixel(op->format_in) / BITS_PER_BYTE;
	uint8_t *line;
 ...
 * @return The pointer to the input data.
 */
static inline const uint8_t *mpix_operation_get_all_input(struct mpix_operation *op)
{
	uint8_t *remaining;
	uint32_t size;
...
static inline void mpix_operation_run(struct mpix_operation *op)
{
...
/**
 * @brief Mark the line obtained with @ref mpix_operation_get_output_line as converted.
 *
 ...
 * @param op Current operation in progress.
 */
static inline void mpix_operation_done(struct mpix_operation *op)
{
	...
	mpix_operation_run(SYS_SLIST_PEEK_NEXT_CONTAINER(op, node));
