...
#include <mpix/bayer.h>
#include <zephyr/logging/log.h>

LOG_MODULE_REGISTER(mpix_bayer, CONFIG_MPIX_LOG_LEVEL);
...
int mpix_image_debayer(struct mpix_image *img, uint32_t window_size)
{
	...
	STRUCT_SECTION_FOREACH_ALTERNATE(mpix_convert, mpix_operation, tmp) {
		if (tmp->format_in == img->format && tmp->format_out == MPIX_FORMAT_RGB24 &&
	...
	if (op == NULL) {
		LOG_ERR("Conversion operation from %s to %s using %ux%u window not found",
			MPIX_FORMAT_TO_STR(img->format), VIDEO_FOURCC_TO_STR(MPIX_FORMAT_RGB24),
...
__weak void mpix_line_rggb8_to_rgb24_3x3(const uint8_t *i0, const uint8_t *i1,
...
__weak void mpix_line_grbg8_to_rgb24_3x3(const uint8_t *i0, const uint8_t *i1,
					     const uint8_t *i2, uint8_t *o0, uint16_t w)
{
	__ASSERT_NO_MSG(w >= 4 && w % 2 == 0);
...
__weak void mpix_line_bggr8_to_rgb24_3x3(const uint8_t *i0, const uint8_t *i1,
					     const uint8_t *i2, uint8_t *o0, uint16_t w)
{
	__ASSERT_NO_MSG(w >= 4 && w % 2 == 0);
	uint8_t il[3][4] = FOLD_L_3X3(i0, i1, i2);
...
__weak void mpix_line_gbrg8_to_rgb24_3x3(const uint8_t *i0, const uint8_t *i1,
					     const uint8_t *i2, uint8_t *o0, uint16_t w)
{
	__ASSERT_NO_MSG(w >= 4 && w % 2 == 0);
	uint8_t il[3][4] = FOLD_L_3X3(i0, i1, i2);
...
__weak void mpix_line_rggb8_to_rgb24_2x2(const uint8_t *i0, const uint8_t *i1, uint8_t *o0,
					     uint16_t w)
{
...
__weak void mpix_line_gbrg8_to_rgb24_2x2(const uint8_t *i0, const uint8_t *i1, uint8_t *o0,
					     uint16_t w)
{
	__ASSERT_NO_MSG(w >= 2 && w % 2 == 0);
...
__weak void mpix_line_bggr8_to_rgb24_2x2(const uint8_t *i0, const uint8_t *i1, uint8_t *o0,
					     uint16_t w)
{
	__ASSERT_NO_MSG(w >= 2 && w % 2 == 0);
...
__weak void mpix_line_grbg8_to_rgb24_2x2(const uint8_t *i0, const uint8_t *i1, uint8_t *o0,
					     uint16_t w)
{
	__ASSERT_NO_MSG(w >= 2 && w % 2 == 0);
...
static inline void mpix_op_bayer_to_rgb24_3x3(struct mpix_operation *op, fn_3x3_t *fn0,
							fn_3x3_t *fn1)
{
	uint16_t prev_line_offset = op->line_offset;
	const uint8_t *i0 = mpix_operation_get_input_line(op);
	const uint8_t *i1 = mpix_operation_peek_input_line(op);
	const uint8_t *i2 = mpix_operation_peek_input_line(op);

	if (prev_line_offset == 0) {
		fn1(i1, i0, i1, mpix_operation_get_output_line(op), op->width);
		mpix_operation_done(op);
	}

	if (prev_line_offset % 2 == 0) {
		fn0(i0, i1, i2, mpix_operation_get_output_line(op), op->width);
		mpix_operation_done(op);
	} else {
		fn1(i0, i1, i2, mpix_operation_get_output_line(op), op->width);
		mpix_operation_done(op);
	}

	if (op->line_offset + 2 == op->height) {
		fn0(i1, i2, i1, mpix_operation_get_output_line(op), op->width);
		mpix_operation_done(op);
...
static inline void mpix_op_bayer_to_rgb24_2x2(struct mpix_operation *op, fn_2x2_t *fn0,
							fn_2x2_t *fn1)
{
	uint16_t prev_line_offset = op->line_offset;
	const uint8_t *i0 = mpix_operation_get_input_line(op);
	...
	if (prev_line_offset % 2 == 0) {
		fn0(i0, i1, mpix_operation_get_output_line(op), op->width);
		mpix_operation_done(op);
	} else {
		fn1(i0, i1, mpix_operation_get_output_line(op), op->width);
		mpix_operation_done(op);
	}
...
static void mpix_op_rggb8_to_rgb24_3x3(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_3x3(op, &mpix_line_rggb8_to_rgb24_3x3,
				    &mpix_line_gbrg8_to_rgb24_3x3);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_rggb8_to_rgb24_3x3, SRGGB8, 3);

static void mpix_op_gbrg8_to_rgb24_3x3(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_3x3(op, &mpix_line_gbrg8_to_rgb24_3x3,
				    &mpix_line_rggb8_to_rgb24_3x3);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_gbrg8_to_rgb24_3x3, SGBRG8, 3);

static void mpix_op_bggr8_to_rgb24_3x3(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_3x3(op, &mpix_line_bggr8_to_rgb24_3x3,
				    &mpix_line_grbg8_to_rgb24_3x3);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_bggr8_to_rgb24_3x3, SBGGR8, 3);

static void mpix_op_grbg8_to_rgb24_3x3(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_3x3(op, &mpix_line_grbg8_to_rgb24_3x3,
				    &mpix_line_bggr8_to_rgb24_3x3);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_grbg8_to_rgb24_3x3, SGRBG8, 3);

static void mpix_op_rggb8_to_rgb24_2x2(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_2x2(op, &mpix_line_rggb8_to_rgb24_2x2,
				    &mpix_line_gbrg8_to_rgb24_2x2);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_rggb8_to_rgb24_2x2, SRGGB8, 2);

static void mpix_op_gbrg8_to_rgb24_2x2(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_2x2(op, &mpix_line_gbrg8_to_rgb24_2x2,
				    &mpix_line_rggb8_to_rgb24_2x2);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_gbrg8_to_rgb24_2x2, SGBRG8, 2);

static void mpix_op_bggr8_to_rgb24_2x2(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_2x2(op, &mpix_line_bggr8_to_rgb24_2x2,
				    &mpix_line_grbg8_to_rgb24_2x2);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_bggr8_to_rgb24_2x2, SBGGR8, 2);

static void mpix_op_grbg8_to_rgb24_2x2(struct mpix_operation *op)
{
	mpix_op_bayer_to_rgb24_2x2(op, &mpix_line_grbg8_to_rgb24_2x2,
				    mpix_line_bggr8_to_rgb24_2x2);
}
MPIX_DEFINE_BAYER_OPERATION(mpix_op_grbg8_to_rgb24_2x2, SGRBG8, 2);
