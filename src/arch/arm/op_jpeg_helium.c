...
void mpix_jpeg_encode_op(struct mpix_base_op *base)
{
    ...
    // Process a line of MCUs (8x8 for 4:4:4; 16x16 for 4:2:0 as configured in encoder)
    src = mpix_op_get_input_lines(base, 8);
    for (int i = 0; i < base->width; i += op->encoder.cx) {
        ret = JPEGAddMCU_Helium(&op->image, &op->encoder, (uint8_t*)&(src[i*bytespp]), pitch);
        if (ret != JPEGE_SUCCESS) {
    ...
    if (base->line_offset == base->height) {
        MPIX_INF("JPEG frame conversion complete (Helium)");
