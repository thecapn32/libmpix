...
static const uint8_t mpix_idx_rggb8[4] = {MPIX_IDX_R, MPIX_IDX_G, MPIX_IDX_G, MPIX_IDX_B};
static const uint8_t mpix_idx_bggr8[4] = {MPIX_IDX_B, MPIX_IDX_G, MPIX_IDX_G, MPIX_IDX_R};
static const uint8_t mpix_idx_gbrg8[4] = {MPIX_IDX_G, MPIX_IDX_B, MPIX_IDX_R, MPIX_IDX_G};
static const uint8_t mpix_idx_grbg8[4] = {MPIX_IDX_G, MPIX_IDX_R, MPIX_IDX_B, MPIX_IDX_G};

/* Extract a random value from the buffer */

static inline uint32_t mpix_rand(void)
{
...
static inline void mpix_sample_rgb24(const uint8_t *buf, size_t size, uint8_t rgb24[3])
{
	uint32_t pos = mpix_rand() % size;
...
static inline void mpix_sample_bayer(const uint8_t *buf, size_t size, uint16_t width,
...
static inline void mpix_sums_to_rgb24avg(uint32_t sums[3], uint8_t rgb24avg[3], uint16_t nval)
{
...
static inline void mpix_sums_add_rgb24(uint32_t sums[3], uint8_t rgb24[3])
{
...
void mpix_rgb24frame_to_rgb24avg(const uint8_t *buf, size_t size, uint8_t rgb24avg[3],
				  uint16_t nval)
{
	...
	for (uint16_t n = 0; n < nval; n++) {
		mpix_sample_rgb24(buf, size, rgb24);
		mpix_sums_add_rgb24(sums, rgb24);
	}
	mpix_sums_to_rgb24avg(sums, rgb24avg, nval);
}

static inline void mpix_bayerframe_to_rgb24avg(const uint8_t *buf, size_t size, uint16_t width,
						...
						const uint8_t *idx)
{
	...
	for (uint16_t n = 0; n < nval; n++) {
		mpix_sample_bayer(buf, size, width, rgb24, idx);
		mpix_sums_add_rgb24(sums, rgb24);
	}
	mpix_sums_to_rgb24avg(sums, rgb24avg, nval);
}
...
void mpix_rggb8frame_to_rgb24avg(const uint8_t *buf, size_t size, uint16_t width,
				  uint8_t rgb24avg[3], uint16_t nval)
{
	mpix_bayerframe_to_rgb24avg(buf, size, width, rgb24avg, nval, mpix_idx_rggb8);
}

void mpix_bggr8frame_to_rgb24avg(const uint8_t *buf, size_t size, uint16_t width,
				  uint8_t rgb24avg[3], uint16_t nval)
{
	mpix_bayerframe_to_rgb24avg(buf, size, width, rgb24avg, nval, mpix_idx_bggr8);
}

void mpix_gbrg8_to_rgb24avg(const uint8_t *buf, size_t size, uint16_t width,
				  uint8_t rgb24avg[3], uint16_t nval)
{
	mpix_bayerframe_to_rgb24avg(buf, size, width, rgb24avg, nval, mpix_idx_gbrg8);
}

void mpix_grbg8frame_to_rgb24avg(const uint8_t *buf, size_t size, uint16_t width,
				  uint8_t rgb24avg[3], uint16_t nval)
{
	mpix_bayerframe_to_rgb24avg(buf, size, width, rgb24avg, nval, mpix_idx_grbg8);
}

/* RGB24 histogram statistics */

static inline void mpix_rgb24hist_add_rgb24(uint16_t *rgb24hist, uint8_t rgb24[3],
					     uint8_t bit_depth)
{
				   ...
				   size_t hist_size, uint16_t nval)
{
	...
	for (uint16_t n = 0; n < nval; n++) {
		mpix_sample_rgb24(buf, buf_size, rgb24);
		mpix_rgb24hist_add_rgb24(rgb24hist, rgb24, bit_depth);
	}
}

static inline void mpix_bayerframe_to_rgb24hist(const uint8_t *buf, size_t buf_size,
						 ...
						 const uint8_t *idx)
{
	...
	for (uint16_t n = 0; n < nval; n++) {
		mpix_sample_bayer(buf, buf_size, width, rgb24, idx);
		mpix_rgb24hist_add_rgb24(rgb24hist, rgb24, bit_depth);
	}
}
...
{
	mpix_bayerframe_to_rgb24hist(buf, buf_size, width, rgb24hist, hist_size, nval,
				      mpix_idx_rggb8);
}
...
static inline void mpix_y8hist_add_y8(uint16_t *y8hist, uint8_t y8, uint8_t bit_depth)
{
				...
				size_t hist_size, uint16_t nval)
{
	...
	for (uint16_t n = 0; n < nval; n++) {
		mpix_sample_rgb24(buf, buf_size, rgb24);
		mpix_y8hist_add_y8(y8hist, mpix_rgb24_get_luma_bt709(rgb24), bit_depth);
	}
}

static inline void mpix_bayerframe_to_y8hist(const uint8_t *buf, size_t buf_size, uint16_t width,
...
void mpix_rggb8frame_to_y8hist(const uint8_t *buf, size_t buf_size, uint16_t width,
				uint16_t *y8hist, size_t hist_size, uint16_t nval)
{
	mpix_bayerframe_to_y8hist(buf, buf_size, width, y8hist, hist_size, nval, mpix_idx_rggb8);
}

void mpix_gbrg8frame_to_y8hist(const uint8_t *buf, size_t buf_size, uint16_t width,
				uint16_t *y8hist, size_t hist_size, uint16_t nval)
{
	mpix_bayerframe_to_y8hist(buf, buf_size, width, y8hist, hist_size, nval, mpix_idx_gbrg8);
}

void mpix_bggr8frame_to_y8hist(const uint8_t *buf, size_t buf_size, uint16_t width,
				uint16_t *y8hist, size_t hist_size, uint16_t nval)
{
	mpix_bayerframe_to_y8hist(buf, buf_size, width, y8hist, hist_size, nval, mpix_idx_bggr8);
}

void mpix_grbg8frame_to_y8hist(const uint8_t *buf, size_t buf_size, uint16_t width,
				uint16_t *y8hist, size_t hist_size, uint16_t nval)
{
	mpix_bayerframe_to_y8hist(buf, buf_size, width, y8hist, hist_size, nval, mpix_idx_grbg8);
}
