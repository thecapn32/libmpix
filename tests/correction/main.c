...
void test_some_matrix(void)
{
	...
	mpix_test_equal(dst[0], (src[0] * correction.color_matrix.levels[0] +
				 src[1] * correction.color_matrix.levels[1] +
				 src[2] * correction.color_matrix.levels[2]));

	mpix_test_equal(dst[1], (src[0] * correction.color_matrix.levels[3] +
				 src[1] * correction.color_matrix.levels[4] +
				 src[2] * correction.color_matrix.levels[5]));

	mpix_test_equal(dst[2], (src[0] * correction.color_matrix.levels[6] +
...
void test_fused_pipeline(void)
{
	...
	struct mpix_correction_all all = {
		.white_balance =
			{
				.red_level = 1.25 * (1 << MPIX_CORRECTION_SCALE_BITS),  /* ~1280 */
				.blue_level = 0.75 * (1 << MPIX_CORRECTION_SCALE_BITS), /* ~768 */
			},
		.color_matrix =
			{
	...
	union mpix_correction_any wb;
	wb.white_balance.red_level = all.white_balance.red_level;
	wb.white_balance.blue_level = all.white_balance.blue_level;
	union mpix_correction_any cm;
	for (int i = 0; i < 9; ++i) {
		cm.color_matrix.levels[i] = all.color_matrix.levels[i];
	}
